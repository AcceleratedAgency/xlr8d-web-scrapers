...
async function configureMessageBus() {
  await messageBus
    .getQueue(service_config.QUEUE_TASK_TYPE.SCRAPING)
    .then(({ recv }) =>
      recv((data, channel, msg) => {
...
async function storeData(slug, db_collection, data) {
  ...
  return (
    await mongo_client.db(slug).collection(db_collection).insertOne(data)
  ).insertedId.toString();
}
let cancelled = !1;
async function startScraping(task, endProcessDelay) {
  ...
  await messageBus
    .subscribe(service_config.QUEUE_TASK_TYPE.SCRAPING + ".cancel." + id)
    .then((handle) =>
      handle((task) => {
        if (cancelled) return;
        ...
        taskFinished(id)
          .catch(console.error)
          .finally(() => endProcess("Job cancelled"));
        cancelled = !0;
      })
    )
    .catch(console.error);
  let sub = taskSubject.subscribe({
    next: async (data) => {
      endProcessDelay();
      log(`Response from scraper: \n`, data);
      let db_doc_id = await storeData(
        slug,
        service_config.QUEUE_TASK_TYPE.SCRAPING,
        data
      ).catch(console.error);
      if (!db_doc_id) return;
      await messageBus
        .getQueue(service_config.QUEUE_TASK_TYPE.CLASSIFY)
        .then(({ send }) =>
          send({
            ...task,
